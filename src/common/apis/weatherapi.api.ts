...
export default class WeatherApi extends baseApi {
  ...
  getWeather(lat: number, lon: number) {
    return this.Get<WeatherApiData>(
      `current.json?key=cb460d7d9f6048f694671115241504&q=${lat.toFixed(4)},${lon.toFixed(4)}&aqi=no&lang=vi`
    )
  }

  getForecast(lat: number, lon: number, days = 7) {
    return this.Get<WeatherApiDataWithForecast>(
      `forecast.json?key=cb460d7d9f6048f694671115241504&q=${lat.toFixed(4)},${lon.toFixed(4)}&aqi=no&days=${days}&lang=vi`
    )
  }
}
